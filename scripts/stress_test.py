...
def main() -> None:
    ...
    assert_true("guestOfferModal" in app_js and 'a[href="/login"], a[href="/signup"]' in app_js, "signed-out auth links should show the guest referral offer before login/signup")
    assert_true("REFER_DUDE143" in app_js and "Referral deal loaded" in app_js, "guest offer should load the referral code into booking search when possible")
    ...
    assert_true(".guest-offer-backdrop" in styles and "max-height: 88vh" in styles, "guest offer modal should be styled and mobile safe")
    index_template = (ROOT / "templates" / "index.html").read_text(encoding="utf-8")
    ...
    assert_true("$guest_offer_modal" in index_template and "$guest_offer_modal" in (ROOT / "templates" / "auth.html").read_text(encoding="utf-8"), "public pages should render the reusable guest offer modal")
    assert_true("data-auto-show" in (ROOT / "app.py").read_text(encoding="utf-8"), "login/signup pages should be able to auto-show the guest offer")
