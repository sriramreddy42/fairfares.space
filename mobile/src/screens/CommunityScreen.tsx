...
export function CommunityScreen({ user, city, cars, onRequireLogin, onRequireSignup, onOpenHousing, onCreateHousingPost, onOpenRides, onOpenRentalCars, onOpenGas, onOpenCommunity, onBottomTabsHiddenChange, initialPostId = "", onInitialPostOpened }: Props) {
  ...
  const renderAnswer = (item: CommunityAnswer, replies: CommunityAnswer[] = []) => (
    <View key={item.id} style={[styles.answerCard, item.accepted && styles.acceptedCard]}>
      ...
      {replies.length ? <View style={styles.inlineReplies}>{replies.map((reply) => <View key={reply.id} style={styles.inlineReply}><View style={styles.inlineReplyHead}><UserAvatar photoUrl={reply.author.photoUrl} style={styles.inlineReplyAvatar} imageStyle={styles.avatarImage} /><View style={styles.postAuthor}><Text style={styles.inlineReplyAuthor}>{reply.author.name}</Text><Text style={styles.meta}>{relativeTime(reply.createdAt)}</Text></View></View><Text style={styles.inlineReplyBody}>{reply.body}</Text><View style={styles.answerActions}>{reactionPicker(`answer-${reply.id}`, reply.viewerReaction, reply.reactionCount, (reaction) => void reactAnswer(reply, reaction))}<TouchableOpacity onPress={() => { setAnswerReplyTarget({ id: item.id, name: reply.author.name }); setAnswer(""); }}><Text style={styles.replyAction}>Reply</Text></TouchableOpacity></View></View>)}</View> : null}
    </View>
  );
  ...
  return <View style={styles.screen}>
    ...
    <Modal visible={Boolean(detail)} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setDetail(null)}>
      ...
      <ScrollView contentContainerStyle={styles.detailContent} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" automaticallyAdjustKeyboardInsets>{detail ? <>
        ...
        {detail.canAnswer ? <View style={styles.answerComposer}>{answerReplyTarget ? <View style={styles.replyingTo}><Text style={styles.replyingToText}>Replying to {answerReplyTarget.name}</Text><TouchableOpacity onPress={() => setAnswerReplyTarget(null)}><Text style={styles.replyingToClose}>×</Text></TouchableOpacity></View> : null}{!user ? <View style={styles.guestAllowance}><Text style={styles.guestAllowanceName}>{guestIdentity || "Guest"}</Text><Text style={styles.guestAllowanceCount}>{guestRemaining} of 6 guest messages left</Text></View> : null}<TextInput style={styles.answerInput} value={answer} onChangeText={setAnswer} multiline placeholder={answerReplyTarget ? "Write a reply…" : "Write a comment…"} placeholderTextColor={theme.colors.muted} editable={Boolean(user || guestRemaining > 0)} /><TouchableOpacity style={[styles.sendAnswer, (!answer.trim() || (!user && guestRemaining <= 0)) && styles.disabled]} disabled={!answer.trim() || detailBusy || (!user && guestRemaining <= 0)} onPress={() => void submitAnswer()}><Text style={styles.sendAnswerText}>{detailBusy ? "…" : "Post comment"}</Text></TouchableOpacity>{!user ? <TouchableOpacity onPress={() => setGuestBenefitsOpen(true)}><Text style={styles.guestSignupHint}>Sign up for unlimited comments and replies</Text></TouchableOpacity> : null}</View> : <Text style={styles.locked}>This discussion is closed to new comments.</Text>}
